...
const mobileMenuCls = "flex items-center justify-center h-[65px] gap-[10px] border-b border-[#38365E]"
const mobileHoverCls = "text-[#fff]  bg-[#00000026]"
const mobileTextCls = "drop-shadow-[0px_0px_6px_#FFFFFF80]"
const HomeSvg = () => {
  return <svg className="fill-current" style={{ transform: 'translateY(-6px)' }} width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <g>
...
const FaucetSvg = () => {
  return <svg className="fill-current" width="14" height="16" viewBox="0 0 14 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const CodesSvg = () => {
  return <svg className="fill-current" width="16" height="13" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const ArcadeSvg = () => {
  return <svg className="fill-current" width="17" height="13" viewBox="0 0 17 13" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const MainLayoutHeader = (props: Props) => {
  ...
  const [gameEntryShow, setGameEntryShow] = useState(false)
  ...
  return (
    ...
    >
      ...
      <div className={clsx("flex-1", isMobile ? "w-[100vw] absolute left-0 top-0 right-0" + (isMobileMenuOpen ? " z-[3]" : "") : "flex items-center justify-center relative")}>
        {
          !isMobile && (
            <div className="absolute px-[30px] flex items-center justify-between left-1/2 -translate-x-1/2 top-0 bg-[url('/images/header/menu_bg.svg')] bg-contain bg-no-repeat bg-center w-[806px] h-[52px] font-Unbounded">
              ...
              <div className={clsx(menuCls, (pathname.includes('/dapps') || pathname.includes('/dex/')) ? hoverCls : '')} onClick={() => { pathname !== '/dapps' && router.push("/dapps") }}>
                <DappsSvg />
                <div className={clsx(textCls, '')}>DApps</div>
                {(pathname.includes('/dapps') || pathname.includes('/dex/')) && <LineBottom />}
              </div>
              ...
              <div className={clsx(menuCls, pathname.includes('/arcade') ? hoverCls : '')} onClick={() => { pathname !== '/arcade' && router.push("/arcade") }}>
                <ArcadeSvg />
                <div className={clsx(textCls, '')}>Arcade</div>
                {pathname.includes('/arcade') && <LineBottom />}
              </div>
            </div>
          )
        }

        {
          isMobile && (
            <motion.div
              ...
              initial={{ height: 0 }}
              animate={{ height: isMobileMenuOpen ? "auto" : 0, paddingBottom: isMobileMenuOpen ? '20px' : 0 }}
              transition={{ duration: 0.3, ease: "easeInOut" }}
            >
              <div className={clsx(mobileMenuCls, pathname === '/' ? mobileHoverCls : '')} onClick={() => { router.push('/') }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", pathname === '/' ? mobileTextCls : '')}>
                  <div className="translate-x-[-10px] translate-y-[6px]"><HomeSvg /></div>
                  <div className={clsx(textCls, 'translate-x-[-18px]')}>Hall</div>
                </div>
                {pathname === '/' && <LineBottom />}
              </div>
              <div className={clsx(mobileMenuCls, pathname === '/faucet' ? mobileHoverCls : '')} onClick={() => { router.push('/faucet') }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", pathname === '/faucet' ? mobileTextCls : '')}>
                  <FaucetSvg />
                  <div className={clsx(textCls, '')}>Faucet</div>
                </div>
              </div>
              <div className={clsx(mobileMenuCls, pathname === '/codes' ? mobileHoverCls : '')} onClick={() => { router.push('/codes') }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", pathname === '/codes' ? mobileTextCls : '')}>
                  <CodesSvg />
                  <div className={clsx(textCls, '')}>Codes</div>
                </div>
              </div>
              <div className={clsx(mobileMenuCls, (pathname.includes('/dapps') || pathname.includes('/dex/')) ? mobileHoverCls : '')} onClick={() => { router.push('/dapps') }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", (pathname.includes('/dapps') || pathname.includes('/dex/')) ? mobileTextCls : '')}>
                  <DappsSvg />
                  <div className={clsx(textCls, '')}>DApps</div>
                </div>
              </div>
              <div className={clsx(mobileMenuCls, pathname === '/marketplace' ? mobileHoverCls : '')} onClick={() => { router.push('/marketplace') }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", pathname === '/marketplace' ? mobileTextCls : '')}>
                  <TokenSvg />
                  <div className={clsx(textCls, '')}>Tokens</div>
                </div>
              </div>
              <div className={clsx(mobileMenuCls, pathname.includes('/arcade') ? mobileHoverCls : '')} onClick={() => {
                setGameEntryShow(true)
              }}>
                <div className={clsx("flex items-center justify-center gap-[10px]", pathname.includes('/arcade') ? mobileTextCls : '')}>
                  <ArcadeSvg />
                  <div className={clsx(textCls, '')}>Arcade</div>
                </div>
              </div>
        ...
        <div
          ...
          className={clsx(
            ...
            isMobile
              ? "w-[160px] h-[40px] bg-[url('/images/mobile/logo.svg')] mx-auto mt-[-2px]"
      ...
      {isMobile && gameEntryShow && <GameEntry onClose={() => {
        setGameEntryShow(false)
      }} />}
    </header>
  );
};
