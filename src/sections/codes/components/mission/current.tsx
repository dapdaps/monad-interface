...
const CurrentMission = (props: any) => {
  ...
  const { title, action_type, token, token_amount, times } =
    current_round_quest;

  const currentRoundQuestText = useMemo(() => {
    if (!current_round_missions || current_round_missions.length === 0)
      return null;
    const questMissionLinks = Object.values(QuestMissionLinks);

    return current_round_missions.map(
      (mission: { title: string; complete: boolean }, index: number) => {
        ...
        for (const link of questMissionLinks) {
          if (link.reg.test(text)) {
            ...
            return (
              <div
                key={index}
                className="flex items-center gap-[5px] md:justify-center"
              >
                <div
                  className={clsx(
  ...
  return (
    <div
      className={clsx(
        "w-full p-[13px_15px_12px_15px] rounded-[6px] border border-[#26274B] bg-[#31305A] flex items-center justify-between gap-[16px] text-white font-Unbounded text-[14px]",
        className
      )}
    >
      <div
        className={clsx(
          "flex-1 min-h-[17px] leading-[200%] md:w-full",
          isMobile && missionLoading ? "flex justify-center" : ""
        )}
      >
        {missionLoading ? (
      ...
      <div
        className={clsx(
          "flex items-center gap-[12px] shrink-0",
          codesClassName
        )}
      >
        <div className="flex items-center gap-[2px]">
          +{missionLoading ? <Loading size={12} /> : currentRountCodes} Codes
        </div>
      </div>
    </div>
  );
