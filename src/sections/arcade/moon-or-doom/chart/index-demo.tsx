...
export default function Chart({ bet, list = [], betList, handleBet, betLoading, userBet, winObj, allTimePrice }: { bet: number, list: any[], betList: any, handleBet: (bet: any) => void, betLoading: boolean, userBet: any, winObj: any, allTimePrice: any }) {
    ...
    useEffect(() => {
        if (betList) {
            ...
            if (betList.bets) {
                betList.bets.forEach((bet: any) => {
                    const bets = JSON.parse(bet.bets)
                    bets.forEach((betItem: any) => {
                ...
                betList.new_bets.forEach((bet: any) => {
                    const bets = JSON.parse(bet.bets)
                    bets.forEach((betItem: any) => {
                        if (betObj[bet.start_time + '-' + betItem.min_price]) {
                            betObj[bet.start_time + '-' + betItem.min_price].new_multiplier = betItem.multiplier;
                            return;
                        }
                        betObj[bet.start_time + '-' + betItem.min_price] = {
                            new_multiplier: betItem.multiplier,
                            ...betItem,
                            source_time: bet.source_time,
                        };
                    });
            ...
            console.log('betList betObj: ', betObj)
    ...
    useThrottleEffect(() => {
        ...
        futureGridRects.each(function () {
            ...
            if (gridTimeStr) {
                ...
                let betTextNew: any = chartGroup.select('.future-grid').select('.new-bet-text-' + className);
                const betMultiplier = betRef.current?.[key]?.multiplier ?? 0;
                const newBetMultiplier = betRef.current?.[key]?.new_multiplier ?? 0;
                ...
                if (betText.empty() && (betMultiplier > 0 || userBetRef.current?.[key])) {
                    ...
                    betText = chartGroup.select('.future-grid').append<SVGTextElement>('text')
                        ...
                        .attr('y', Number(d3.select(this).attr('y')) + configRef.current?.gridCellSize - padding - 20)
                        .attr('fill', 'red')
                        .attr('font-size', '12px')
                        .attr('font-weight', '500')
                    ...
                    betTextNew = chartGroup.select('.future-grid').append<SVGTextElement>('text')
                        .attr('class', 'new-bet-text-' + className)
                        .attr('x', Number(d3.select(this).attr('x')) + configRef.current?.gridCellSize - padding)
                ...
                if ((betMultiplier > 0 || userBetRef.current?.[key]) && !betText.empty() && ((isPast && userBetRef.current?.[key]) || !isPast)) {
                    ...
                    betTextNew.text(newBetMultiplier + 'x');
                }
