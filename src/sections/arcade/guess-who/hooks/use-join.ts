...
export function useJoin(props?: any) {
  ...
  const [lastMonsters] = useMemo(() => {
    return [
      Object.values(MONSTERS)
        ...
        .filter(
          (it) =>
            !room?.players?.some((player: any) => player.moves === it.value)
        )
    ];
  }, [room]);
  ...
  const { runAsync: onJoinCheck, loading: checking } = useRequest(
    async (_room: Room, options?: { isReset?: boolean }) => {
      ...
      // check your own game
      if (
        account.toLowerCase() === _room.address.toLowerCase() &&
        _room.players.length > 1 &&
        _room.players.every(
          (player) => player.address.toLowerCase() === account.toLowerCase()
        )
      ) {
        toast.fail({
          title: "Join failed",
      ...
      if (
        newRoomInfo.players.length >= 3 ||
        newRoomInfo.contractData?.status !== ContractStatus.Ongoing
      ) {
        toast.fail({
          title: "Join failed",
          text: "This room has ended, please refresh the list and try again."
      ...
      if (
        account.toLowerCase() === newRoomInfo.address.toLowerCase() &&
        newRoomInfo.players.length > 1 &&
        newRoomInfo.players.every(
          (player: any) =>
  ...
  const { runAsync: onJoin, loading: joining } = useRequest(
    async () => {
      ...
      let params = [room?.room_id, betMonster[0], userRandomNumber];
      if (isDouble) {
        params = [
          room?.room_id,
          betMonster[0],
      ...
      try {
        ...
        const handleResult = (_newRoomInfo: any) => {
          if (_newRoomInfo.winner_address) {
            ...
            if (
              _newRoomInfo.winner_address?.toLowerCase() ===
              account?.toLowerCase()
            ) {
