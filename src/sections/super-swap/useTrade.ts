...
const templates = [swap.oneclick, swap.pancake, swap.uniswap, swap.lfj, swap.kuru];
// const templates = [swap.oneclick];
export default function useTrade({ chainId, template, from, onSuccess }: any) {
      ...
      try {
        ...
        if (wrapType) {
          ...
          const tradeList = [{
            ...
            noPair: false,
            name: swap.oneclick.name,
            icon: swap.oneclick.logo,
            txn,
        ...
        const _list: any = []
        const allDataPromises = templates.map((template) => {
          ...
          const data = quoter(params).then(res => {
            if (res?.outputCurrencyAmount && Big(res.outputCurrencyAmount).gt(0)) {
              const _trade: any = {
                ...formatTrade({
                  market: { ...res, template: template.name, icon: template.logo },
                  rawBalance,
                  gasPrice,
                  prices,
                  inputCurrency,
              ...
              console.log('_trade: %o', _trade);
  
              _list.push(_trade);
  
              _list.sort((a: any, b: any) => b.outputCurrencyAmount - a.outputCurrencyAmount);
              console.log('template: %s tradeList: %o', template.name, _list);
              setTradeList([
                ..._list
              ]);
              setLoading(false);
              setSelectedRoute(_list[0]);
        ...
        if (typeof template === "string") {
          ...
          tradeList.sort((a: any, b: any) => b.outputCurrencyAmount - a.outputCurrencyAmount);

          console.log('tradeList: %o', tradeList);
  ...
  const onSwap = useCallback(async (trade: any) => {
    if (!provider) return;

    console.log('trade: %o', trade);
    ...
    try {
      ...
      if (template === "Pancake" && !trade.wrapType) {
        tx = await onSwapUniversal({ signer, toastId, trade });
      } else {
  ...
  const onSwapUniversal = useCallback(async ({ signer, toastId, trade }: any) => {
