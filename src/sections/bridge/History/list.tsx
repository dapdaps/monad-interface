  ...
  return (
    <div className={cls}>
      <div className="rounded-2xl p-4 relative">
        <div
          className="absolute top-[20px] cursor-pointer right-[10px]"
        ...
        >
          ...
          >
            <path
              d="M14 2.19995L8 6.99995L2 2.19995"
              stroke="black"
              strokeWidth="3"
        ...
        <h2 className="text-black text-[16px] font-[600] text-xl mb-3">
          My transactions
        </h2>
        <div className="flex gap-3 mb-4 bg-white border border-[#000] rounded-xl p-1">
          <div
            className={`flex flex-1 items-center gap-2 cursor-pointer rounded-xl px-4 py-2 ${
              activeTab === "pending" ? "bg-[#FFDC50]" : ""
            }`}
          ...
          <div
            className={`flex flex-1 items-center gap-2 cursor-pointer rounded-xl  px-4 py-2 ${
              activeTab === "history" ? "border border-[#000] bg-[#FFDC50]" : ""
            }`}
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M5.7998 6.99976H12.9998"
                stroke="#76500D"
                strokeWidth="2"
                stroke-linecap="round"
              />
              <path
                d="M5.7998 11.8003H10.5998"
                stroke="#76500D"
                strokeWidth="2"
                stroke-linecap="round"
              />
...
function HistoryItem({ item }: { item: any }) {
  ...
  let toToken =
    _allTokens[item.to_chain_id]?.[
      tokenPairs[item.chain_id]?.[action_tokens[0].toUpperCase()]?.toUpperCase()
    ];

  if (
    Number(item.chain_id) === 80094 &&
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              <div className="w-[30px] h-[30px] relative">
                ...
                />
                <img
                  className="w-[10px] h-[10px] object-contain border border-[#000] rounded-full absolute bottom-0 right-0"
                  src={chains[item.chain_id].icon}
                />
              </div>
              <div>
                {balanceFormated(item.action_amount)}
                <br />
                {fromToken?.symbol}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="w-[30px] h-[30px] relative">
                ...
                <img
                  className="w-[10px] h-[10px] object-contain border border-[#000] rounded-full absolute bottom-0 right-0"
                  src={chains[item.to_chain_id].icon}
                />
              </div>
              <div>
                {balanceFormated(item.action_amount)}
          ...
          <div className="text-sm text-gray-500 flex justify-between items-center">
            <div>
              {formatEnglishDate(new Date(item.create_time).getTime())}
              {
                <a
                  target="_blank"
                  href={`${chains[item.chain_id].blockExplorers}/tx/${
                    item.tx_id
                  }`}
            ...
            <div>
              {Number(item.bridge_status) !== 4 && (
                <span className="text-[#006BFF]">Processing~3min</span>
              )}
              {Number(item.bridge_status) === 4 && (
                <span className="text-[#006BFF]">Success</span>
              )}
            </div>
          </div>
        </div>
