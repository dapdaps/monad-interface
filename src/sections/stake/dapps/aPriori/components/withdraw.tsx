...
export default function Stake() {
  ...
  const { run: throttledGetConvertToAssets } = useThrottleFn(
    (amount) => {
      if (!amount) return;
    ...
    <div className="pt-4 relative">
        ...
        <div className="flex justify-between items-center mb-2">
          <input
            ...
            onChange={(e) => {
              ...
              setAmount(val);
              const _percent =
                (Number(val) / Number(balances[APRIORI_CONTRACT_ADDRESS])) *
                100;
          ...
          <div className="flex flex-col items-end">
            ...
            <span
              ...
              className="text-[10px] cursor-pointer text-[#B6B3D6] underline mt-[5px]"
            >
              balance: {balanceFormated(balances[APRIORI_CONTRACT_ADDRESS])}
            </span>
          </div>
        </div>
      ...
      <div className="flex gap-4 mt-6 mb-2">
        ...
        <div className="flex-1  bg-[#23224A] rounded-xl border border-[#B6B3D6] p-5 flex flex-col gap-2 shadow-md">
          <div className="text-[#E7E2FF] text-[14px] font-semibold mb-2">
            Use aPriori
          </div>
        ...
        <div className="flex-1 bg-[#6B6A8B] bg-opacity-50 rounded-xl p-5 flex flex-col gap-2 opacity-60 cursor-not-allowed">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[#C7C6E5] text-[14px] font-semibold">
              Use Pool
            </span>
      ...
      <div className="w-[370px] mx-auto mt-8 bg-[#23224A] rounded-2xl shadow-lg border border-[#3B3970] font-Montserrat absolute top-[-80px] right-[-460px]">
        <div className="flex border-b border-[#35346A]">
          <button
            className={`flex-1 py-2 text-[18px] transition ${
              tabIndex === 0
          ...
          <button
            className={`flex-1 py-2 text-[18px] font-semibold transition ${
              tabIndex === 1
                ? "text-[#A6FF7A] border-b-2 border-[#A6FF7A]"
                : "text-[#A6A6DB]"
            }`}
            ...
            <div className="flex items-center px-6 py-5" key={item.id}>
              ...
              <div className="flex-1">
                <div className="text-white text-[18px] font-semibold">
                  {balanceFormated(
                    new Big(item.assets).div(10 ** 18).toString()
                  )}{" "}
                  <span className="text-[#A6A6DB] text-[14px] font-normal ml-1">
                    MON
                  </span>
                </div>
              </div>
...
const clsCliam =
  "ml-4 px-6 py-1 flex items-center justify-center gap-2 rounded-lg bg-[#8B87FF] text-white text-[16px] font-bold hover:opacity-80 transition";
const ChaimBtn = ({
  item,
  handleClaim,
  getWithdrawalRequests,
...
const TimeItem = ({
  ...
  useInterval(() => {
    setTime(
      10 -
        Math.floor(
          (new Date().getTime() - item.requested_at * 1000) / 1000 / 60
        )
    );
  }, 1000);
...
const cls =
  "w-full h-[60px] flex items-center justify-center rounded-[6px] text-[#fff] bg-[#8B87FF] text-[18px] font-[600] mt-[16px] cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed";
...
const WithdrawBtn = ({
  ...
  return (
    <button
      disabled={
        isStakeLoading ||
        Number(amount) > Number(balances[APRIORI_CONTRACT_ADDRESS]) ||
        Number(amount) <= 0
      }
      onClick={async () => {
