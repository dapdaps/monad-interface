    ...
    return (
        <div className="font-DogicaPixel bg-[#6D7EA524] mx-[25px] px-[15px] py-[10px] rounded-[4px] mb-[40px] mt-[10px]">
            <div className="flex items-center gap-[10px] justify-between w-full">
                <div className="text-[#A6A6DB]">* Hold an ecosystem NFTs</div>
                <motion.svg
                    onClick={() => {
                        getMultiNft();
                    }}
                    ...
                    animate={multiNftLoading ? { rotate: 360 } : { rotate: 0 }}
                    transition={multiNftLoading ? { repeat: Infinity, duration: 1, ease: "linear" } : { duration: 0.01 }}
            ...
            <div className="flex items-center justify-between gap-[10px] mt-[10px]">
                {multiNft.map((item: any, index: number) => (
                    <div
                        key={index}
                        className="flex w-[60px] items-center cursor-pointer flex-col relative group"
                        onClick={() => {
                            window.open(`${baseUri}${NFT_ADDRESSES[index]}`, '_blank');
                        }}
                    >
                        <div className="w-full h-[60px] overflow-hidden rounded-[10px] flex items-center justify-center">
                            <img src={item.tokenURI} alt={item.name} className="object-fill" />
                        </div>
                        <div className="text-[#A6A6DB] w-full text-[10px] mt-[10px] whitespace-nowrap text-center overflow-hidden text-ellipsis">{item.name}</div>
                        {
                            item.balance > 0 && (
                        ...
                        <div className="absolute top-[-45px] left-[50%] translate-x-[-50%] text-[12px] text-[#A6A6DB] whitespace-nowrap p-[10px] bg-[#1A1843CC] rounded-[6px] border border-[#3E347C] opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 z-10">
                            { item.name }
                        </div>
                    </div>
                ))}
