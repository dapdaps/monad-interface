...
export default function Trade({
  ...
  const tokenInAddress = useMemo(() => {
    return tokenIn.isNative || tokenIn.address === "native"
      ? "native"
      : tokenIn.address;
  }, [tokenIn]);
  ...
  const {
    ...
    tokenInAddress,
    tokenIn.decimals,
    tokenIn.chainId || DEFAULT_CHAIN_ID
  );

  const tokenOutAddress = useMemo(() => {
    return tokenOut.isNative || tokenOut.address === "native"
      ? "native"
      : tokenOut.address;
  }, [tokenOut]);
  ...
  const {
    ...
    tokenOutAddress,
    tokenOut.decimals,
    tokenOut.chainId || DEFAULT_CHAIN_ID
  );
  ...
  useEffect(() => {
    ...
    if (Big(amount).gt(currentBalance || 0)) {
      setErrorTips(
        `Insufficient ${
          activeTab === "buy" ? tokenIn?.symbol : tokenOut?.symbol
        } Balance`
      );
      setTrade(null);
    } else {
    ...
    <div className="w-full border border-[#7262FF] rounded-[6px]">
      ...
      <div className="flex items-center gap-0 bg-[#836EF940] h-[clamp(1px,_2.45vw,_calc(var(--pc-1512)*0.0245))] rounded-t-[6px] uppercase text-[clamp(1px,_1.06vw,_calc(var(--pc-1512)*0.0106))] overflow-hidden text-white text-center text-base font-medium ">
        <div
          ...
          style={
            activeTab === "buy"
              ? {
                  background:
                    "radial-gradient(66% 50% at 47.77% 50%, #553BE4 0%, #221662 100%)",
        ...
        <div
          onClick={() => {
            setTokenIn(
              tokenList.find((token) => token.symbol.toUpperCase() === "MON")!
            );
      ...
      <div className="px-[clamp(1px,_1.06vw,_calc(var(--pc-1512)*0.0106))] py-[clamp(1px,_1.62vw,_calc(var(--pc-1512)*0.0162))]">
        ...
        <div className="flex items-center justify-between">
          <div className="text-[clamp(1px,_1.23vw,_calc(var(--pc-1512)*0.0123))]">
            {activeTab === "buy" ? "BUY BC by" : "SELL BC"}
          </div>
            ...
            {activeTab === "buy" ? (
              ...
              >
                <div className="flex items-center justify-end gap-2 shrink-0 border-l border-[#34304B] px-[clamp(1px,_0.66vw,_calc(var(--pc-1512)*0.0066))] h-[clamp(1px,_3.51vw,_calc(var(--pc-1512)*0.0951))] cursor-pointer">
                  ...
                  />
                  <span className="text-white font-semibold text-[clamp(1px,_1.19vw,_calc(var(--pc-1512)*0.0119))]">
                    {currentToken.symbol}
                  </span>
        ...
        <div className="flex items-center justify-between mt-[clamp(1px,_1.93vw,_calc(var(--pc-1512)*0.0193))]">
          <span className="text-[#727D97] text-[clamp(1px,_0.79vw,_calc(var(--pc-1512)*0.0079))]">
            Slippage
          </span>
          <Popover
        ...
        <div className="flex items-center justify-between mt-[clamp(1px,_1.56vw,_calc(var(--pc-1512)*0.116))]">
          ...
          <span className="text-[#BFFF60] text-[clamp(1px,_1.19vw,_calc(var(--pc-1512)*0.0119))] font-medium">
            {balanceFormated(trade?.outputCurrencyAmount, 2)}{" "}
            {outputToken.symbol}
          </span>
        </div>
