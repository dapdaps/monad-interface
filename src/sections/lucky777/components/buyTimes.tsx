...
export const GAME_CONTRACT_ADDRESS_MAINNET =
  "0x395dC2ebf418417096a78FdD18EBC1Ce6c8Fe9FE";
export const GAME_CONTRACT_ADDRESS_PROD =
  "0x7689696eB768100797084167d211865D9Ac9168c";

export const GAME_CONTRACT_ADDRESS =
  process.env.NEXT_PUBLIC_API === "https://mainnet-api-monad.dapdap.net"
...
const BuyTimesModal = ({
  ...
  const handleSelectTimes = useCallback(
    async (selectedTimes: number) => {
      ...
      try {
        ...
        const contract = new Contract(GAME_CONTRACT_ADDRESS, abi, signer);
  ...
  return (
    <Modal open={open} onClose={onClose} className="" closeIcon={<IconClose />}>
      <MonadBaseCard className="w-[592px] md:w-full" contentClassName="">
        <div className="flex flex-col items-center px-[10px]">
          <div className="text-center mt-[-48px]">
            <img
              src="/images/lucky777/buy-777-title.svg"
              alt="LUCKY 777"
              className="w-[183px]  mx-auto"
            />
          </div>
          <div className="w-full bg-[#18172B] rounded-[6px] p-[24px] mt-[42px] border-[#414266] rou">
            <div className="flex justify-between items-center">
              <span className="text-white text-[14px] font-Unbounded">
                TIMES
              </span>
              <span className="text-[#BFFF60] font-bold text-[18px]">
                x
                <input
                  ref={inputRef}
                  value={times}
          ...
          <div className="flex justify-between items-center mt-[15px] text-[14px] font-Unbounded md:w-full">
            <div className="text-[#fff] flex items-center gap-[10px]">
              ...
              >
                ...
                <path
                  d="M12.9999 5.19971C10.7475 5.19971 5.19995 10.7471 5.19995 12.9997C5.19995 15.2522 10.7475 20.7997 12.9999 20.7997C15.2524 20.7997 20.8 15.2522 20.8 12.9997C20.8 10.7472 15.2524 5.19971 12.9999 5.19971ZM11.7844 17.4599C10.8346 17.2011 8.28086 12.734 8.53973 11.7842C8.7986 10.8343 13.2656 8.2806 14.2154 8.53947C15.1653 8.7983 17.7191 13.2653 17.4602 14.2152C17.2013 15.1651 12.7343 17.7188 11.7844 17.4599Z"
                  fill="white"
                />
              </svg>
              MON
            ...
            <div className="text-white">
              {numberFormatter(amount * Number(times), 1, true)}
            </div>
          </div>

          <MainBtn
            isPending={isPending}
            onClick={() => handleSelectTimes(Number(times))}
          />
          ...
          <div className="flex w-full justify-between gap-4">
            <div className="flex-1 bg-[#4D4D73] border-[#ACACE2] rounded-[6px] flex flex-col items-center py-4 gap-2">
              ...
              <img
                src="/images/lucky777/coin-10.svg"
                alt="10 coins"
                className="mb-2 mt-[20px]"
              />
              <MoreBtn onClick={() => handleSelectTimes(10)}>
                {amount * 10} MON
              </MoreBtn>
            </div>
            <div className="flex-1 bg-[#4D4D73] border-[#ACACE2] rounded-[6px] flex flex-col items-center py-4 gap-2">
              ...
              <img
                src="/images/lucky777/coin-50.svg"
                alt="50 coins"
                className="mb-2 mt-[11px]"
              />
              <MoreBtn onClick={() => handleSelectTimes(50)}>
                {amount * 50} MON
              </MoreBtn>
            </div>
            <div className="flex-1 bg-[#4D4D73] border-[#ACACE2] rounded-[6px] flex flex-col items-center py-4 gap-2">
              ...
              <img
                src="/images/lucky777/coin-100.svg"
                alt="100 coins"
                className="mb-2"
              />
              <MoreBtn onClick={() => handleSelectTimes(100)}>
                {amount * 100} MON
              </MoreBtn>
            </div>
          </div>
...
const MainBtn = ({
  ...
  if (!isLogin) {
    return (
      <button
        onClick={() => login()}
        className="w-full flex items-center justify-center gap-2 bg-[#BFFF60] text-[#23223A] text-[14px] py-4 rounded-[6px] mb-8 mt-[30px] border-[#000]"
      >
        Connect Wallet
      </button>
    );
...
const MoreBtn = ({
  ...
  return (
    <button
      ...
      className="bg-[#BFFF60] text-[#23223A] font-bold py-1 px-4 rounded"
    >
      {children}
    </button>
  );
