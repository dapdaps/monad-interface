...
function getTimeLeftToUTC24() {
  ...
  return {
    hours:
      Math.floor(_duration.asHours()) < 10
        ? `0${Math.floor(_duration.asHours())}`
        : Math.floor(_duration.asHours()),
    minutes:
      _duration.minutes() < 10
        ? `0${_duration.minutes()}`
        : _duration.minutes(),
    seconds:
...
export default memo(function Tiger(props: any) {
  ...
  const [WHEEL_SIZE, setWHEEL_SIZE] = useState(
    Number(chogStarrr?.remaining) > 0 ? 600 : 500
  );
  const [SpinCategories, setSpinCategories] = useState(
    Object.values(SPIN_CATEGORIES)
  );
  const [SpinCategoryRotation, setSpinCategoryRotation] = useState(
    WHEEL_AREA / SpinCategories.length
  );
  ...
  useEffect(() => {
    ...
    setSpinCategories(
      Object.values(SPIN_CATEGORIES).filter((it) =>
        prizeStatus.includes(Number(it.code))
      )
    );
  }, [prizeStatus]);
  ...
  const animateCoin = (
    ...
    const moveKeyframes = [
      ...
      {
        transform: `translate(${horizontalDistance * 0.2}px, ${
          maxHeight * 0.3
      ...
      {
        transform: `translate(${horizontalDistance * 0.4}px, ${
          maxHeight * 0.7
      ...
      // Peak point
      {
        ...
        }px, ${maxHeight}px) rotate(${Math.random() * 540}deg)`,
        offset: 0.4
      },
      // Start slow descent
      {
        transform: `translate(${horizontalDistance * 0.8}px, ${
          maxHeight * 0.6
      ...
      // Accelerated descent
      {
        transform: `translate(${horizontalDistance}px, ${Math.abs(
          maxHeight
        )}px) rotate(${Math.random() * 1080}deg)`,
        offset: 1
      }
    ];
    ...
    const removeElement = () => {
      if (
        moveAnimation.playState === "finished" &&
        opacityAnimation.playState === "finished"
      ) {
        coin.remove();
      }
    };
  ...
  const startInfinityScroll: () => Promise<any> = () =>
    new Promise((resolve) => {
      ...
      spinTimerInfinityLeft.current = setTimeout(() => {
        ...
        centerWheelAnimation = centerWheelAnimate(
          centerWheel.current,
          {
            rotateX: [
              centerWheelRotation.get(),
              centerWheelRotation.get() + 360
            ]
          },
          WheelInfinityAnimation
        );
      ...
      spinTimerInfinityCenter.current = setTimeout(() => {
        ...
        rightWheelAnimation = rightWheelAnimate(
          rightWheel.current,
          {
            rotateX: [rightWheelRotation.get(), rightWheelRotation.get() + 360]
          },
  ...
  const startWheelResultScroll: (params: any) => Promise<any> = (params) =>
    new Promise((resolve) => {
      // calc wheel position
      const { draw_codes, category } = params.data;
      const [leftCode, centerCode, rightCode] = draw_codes;

      const leftCategoryIndex = SpinCategories.findIndex(
        (it) => Number(it.code) === Number(leftCode)
      );
      const centerCategoryIndex = SpinCategories.findIndex(
        (it) => Number(it.code) === Number(centerCode)
      );
      const rightCategoryIndex = SpinCategories.findIndex(
        (it) => Number(it.code) === Number(rightCode)
      );
      ...
      const leftWheelCodeRotation =
        baseRotation +
        WHEEL_AREA * leftRandomArea +
        (WHEEL_AREA - leftCategoryIndex * SpinCategoryRotation) -
        1;
      const centerWheelCodeRotation =
        baseRotation +
        WHEEL_AREA * centerRandomArea +
        (WHEEL_AREA - centerCategoryIndex * SpinCategoryRotation) -
        1;
      const rightWheelCodeRotation =
        baseRotation +
        WHEEL_AREA * rightRandomArea +
        (WHEEL_AREA - rightCategoryIndex * SpinCategoryRotation) -
        1;
              ...
              },
              {
                ...
                onComplete: () => {
                  rightWheelAnimate(
                    ...
                    {
                      rotateX: [
                        rightWheelRotation.get(),
                        rightWheelCodeRotation
                      ]
                    },
                    {
                      type: "spring",
  ...
  const { run: handleSpin, loading: spinning } = useRequestByToken<any, any>(
    async () => {
      if (!isLogin) {
        login();
        return;
      ...
      if (
        !spinUserData?.spin_balance ||
        spinUserData?.spin_balance <= 0 ||
        leftSpin <= 0
      ) {
      ...
      if (Number(res.draw_amount) > 0 && !rewardConfig) {
        ...
        if (rewardConfig) {
          ...
          success(
            { title: `WON 1 ${rewardConfig.displayName}` },
            "bottom-right"
          );
  ...
  const playSound = useCallback(
    (soundType: number) => {
      ...
      const soundName =
        soundType === 1 ? "slot-machine-sound" : "winning-sound-effect";
  ...
  return (
    <div className="w-full h-[100dvh] relative flex flex-col items-center justify-center pt-[88px]">
      <div
        style={{
          ...
          transform: `translate(-50%, ${
            isMobile ? "-30px" : "0"
          }) scale(${Math.min(size.height / 1000, 2)})`,
          transformOrigin: "bottom center"
        }}
      >
        <Notice />
        {isMobile && (
          <div className="absolute top-[-30vw] right-[40vw] w-[20vw] h-[20vw]">
            <AllInFire />
          </div>
        )}
        ...
        <div
          className={
            "ml-[10px] overflow-hidden absolute font-HackerNoonV2 text-[60px] leading-[110%] text-[#000] top-[20px] left-1/2 -translate-x-1/2 z-[2] w-[490px] h-[70px] flex flex-col items-center "
          }
        >
          <motion.div
            key={title}
            ...
            className={
              ...
              (title !== DEFAULT_TITLE ? "glitch" : "")
            }
            animate={
              title === DEFAULT_UNLUCKY_TITLE
                ? {
                    x: [1100, -850]
                  }
                : title !== DEFAULT_TITLE
        ...
        <div className="absolute w-[386px] h-[168px] left-1/2 top-[194px] -translate-x-1/2 overflow-hidden">
          ...
          >
            ...
            >
              {new Array(360 / WHEEL_AREA).fill(null).map((_, index) =>
                SpinCategories.map((item, idx) => (
                  <div
                    key={`${index}-${idx}`}
                    className="absolute rounded-full top-1/2 left-1/2 origin-center opacity-100 -mt-[30px] -ml-[30px] [backface-visibility:hidden]"
                    style={{
                      transform: `rotateX(${
                        index * WHEEL_AREA + idx * SpinCategoryRotation
                      }deg) translateZ(${
                        (WHEEL_SIZE * 0.7) / 2
                      }px) translateY(${item.centerY}px)`,
          ...
          <img
            src="/images/lucky777/layer.svg"
            alt=""
            className="w-[121px] h-[168px] z-2 absolute top-0 left-0"
          />
          <img
            src="/images/lucky777/layer.svg"
            alt=""
            className="w-[121px] h-[168px] z-2 absolute top-0 left-1/2 -translate-x-1/2 ml-[2px]"
          />
          <img
            src="/images/lucky777/layer.svg"
            alt=""
            className="w-[121px] h-[168px] z-2 absolute top-0 right-0 mr-[-7px]"
          />
        </div>
        ...
        <div>
          <motion.button
            ...
            animate={{
              backgroundImage:
                leftSpin < 1
                  ? "url('/images/lucky777/multiple/x1-disabled.svg')"
                  : multiple === 1
                  ? "url('/images/lucky777/multiple/x1-press.svg')"
                  : "url('/images/lucky777/multiple/x1.svg')"
            }}
            onClick={() => {
              if (leftSpin < 1) {
                return;
          ...
          >
            <div className="invisible group-hover:visible absolute top-[-140%] z-[10] left-[50%] text-left -translate-x-1/2 border border-[#645E8A] w-[270px] p-[5px_10px] rounded-[6px] text-[#D7D7F6] text-[10px] backdrop-blur-[20px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]">
              Selecting x1 will consume a spin 1 times also your winning prize
              will x1
            </div>
          </motion.button>
          ...
          <motion.button
            ...
            animate={{
              backgroundImage:
                leftSpin < 10
                  ? "url('/images/lucky777/multiple/x10-disabled.svg')"
                  : multiple === 10
                  ? "url('/images/lucky777/multiple/x10-press.svg')"
                  : "url('/images/lucky777/multiple/x10.svg')"
            }}
            onClick={() => {
              if (leftSpin < 10) {
                return;
          ...
          >
            <div className="invisible group-hover:visible absolute top-[-140%] z-[10] left-[50%] text-left -translate-x-1/2 border border-[#645E8A] w-[270px] p-[5px_10px] rounded-[6px] text-[#D7D7F6] text-[10px] backdrop-blur-[20px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]">
              Selecting x10 will consume a spin 10 times also your winning prize
              will x10
            </div>
          </motion.button>
          ...
          <motion.button
            ...
            animate={{
              backgroundImage:
                leftSpin < 50
                  ? "url('/images/lucky777/multiple/x50-disabled.svg')"
                  : multiple === 50
                  ? "url('/images/lucky777/multiple/x50-press.svg')"
                  : "url('/images/lucky777/multiple/x50.svg')"
            }}
            onClick={() => {
              if (leftSpin < 50) {
                return;
          ...
          >
            <div className="invisible group-hover:visible absolute top-[-140%] z-[10] left-[50%] text-left -translate-x-1/2 border border-[#645E8A] w-[270px] p-[5px_10px] rounded-[6px] text-[#D7D7F6] text-[10px] backdrop-blur-[20px] shadow-[0px_0px_10px_0px_rgba(0,0,0,0.25)]">
              Selecting x50 will consume a spin 50 times also your winning prize
              will x50
            </div>
          </motion.button>
        </div>
        ...
        <div
          data-click-sound
          className="absolute bottom-[280px] left-1/2 -translate-x-1/2 z-[2] w-[202px] h-[86px] flex flex-col items-center  max-w-full  bg-top bg-contain bg-no-repeat"
        >
          <motion.button
            data-bp="1009-008"
            ref={spinRef}
            ...
            animate={{
              backgroundImage: pressed3
                ? "url('/images/lucky777/spin-btn-press.svg')"
                : "url('/images/lucky777/spin-btn.svg')"
            }}
            style={{
        ...
        <div>
          <motion.button
            ...
            animate={{
              backgroundImage: pressed2
                ? "url('/images/lucky777/rules-press.svg')"
                : "url('/images/lucky777/rules.svg')"
            }}
            onClick={() => {
              setOpenRules(true);
              setPressed2(true);
          ...
          <motion.button
            ...
            animate={{
              backgroundImage: pressed1
                ? "url('/images/lucky777/history-2-press.svg')"
                : "url('/images/lucky777/history-2.svg')"
            }}
            onClick={() => {
              setOpenHistory(true);
              setPressed1(true);
            ...
            <div className="flex items-center justify-between text-[20px] px-[10px] py-[5px] border-b border-[#A5FFFD5C]">
              ...
              >
                <motion.svg
                  ...
                  animate={
                    spinUserDataLoading ? { rotate: 360 } : { rotate: 0 }
                  }
                  transition={
                    spinUserDataLoading
                ...
                >
                  <path
                    d="M10.7362 18.7455C6.88384 18.7455 3.74969 15.6543 3.74969 11.8543C3.74969 8.05504 6.88384 4.96387 10.7362 4.96387C11.903 4.96387 13.0591 5.25391 14.0796 5.8024C14.5804 6.07225 14.7684 6.69809 14.4985 7.19953C14.2313 7.70184 13.6057 7.88961 13.1021 7.61912C12.381 7.23111 11.5633 7.02637 10.7362 7.02637C8.021 7.02637 5.81219 9.19221 5.81219 11.8543C5.81219 14.5171 8.021 16.683 10.7362 16.683C12.9813 16.683 14.9418 15.1958 15.5029 13.0669C15.6492 12.5165 16.2145 12.1886 16.7639 12.333C17.3143 12.4787 17.6434 13.0426 17.4984 13.5933C16.6966 16.6264 13.9159 18.7455 10.7362 18.7455Z"
                    fill="#6D7EA5"
                  />
                  <path
                    d="M17.1712 8.74491L15.2535 3.84239C15.1203 3.50186 14.6653 3.43891 14.4448 3.73067L11.1229 8.12637C10.9025 8.41813 11.087 8.83879 11.451 8.87403L16.6908 9.38084C17.0378 9.41457 17.2982 9.06975 17.1712 8.74491Z"
                    fill="#6D7EA5"
                  />
                </motion.svg>
                <span className="text-[#A5FFFD]">{leftSpin}</span>
              </div>
            </div>
      ...
      <BuyTimesModal
        ...
        onClose={() => setOpenBuyTimes(false)}
        refreshData={getSpinUserData}
      />
