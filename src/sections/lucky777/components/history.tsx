...
function List({ type, winningOnly, setRewardVisible, setCurrentNft }: { type: string, winningOnly: boolean, setRewardVisible: (visible: boolean) => void, setCurrentNft: (nft: any) => void }) {
    ...
    useEffect(() => {
        if (dataRef.current.length > 0) {
            setData(winningOnly ? dataRef.current.filter((item: any) => (Number(item.amount) > 0) || (Number(item.code) % 111 === 0)) : dataRef.current);
        } else {
            setData([]);
        }
...

                        {
                            type === "payouts" && (
                                <>
                                    ...
                                    {
                                        item.code !== '666' && item.code !== '777' && item.code !== '888' && item.code !== '999' && item.code !== '101010' && item.code !== '111111' && item.code !== '121212' && item.code !== '131313' && item.code !== '141414' && item.code !== '151515' && item.code !== '161616' && item.code !== '171717' && item.code !== '181818' && item.code !== '191919' && (<>
                                            <div className="flex-1 flex items-center gap-2 whitespace-nowrap">
                                                {item.amount} MON
