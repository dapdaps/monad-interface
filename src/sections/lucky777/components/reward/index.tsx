...
import { NFT_INFORMATIONS, RewardShowType } from "@/sections/arcade/space-invaders/config";
...
const Reward = (props: any) => {
  ...
  // const nftInfo = NFT_INFORMATIONS[nft.category.toLowerCase()];

  const [discord, setDiscord] = useState<string>("");

  const isGetNew = false;
  const isNft = false;
  ...
  const { runAsync: handleConfirm, loading: confirmLoading } = useRequest(async () => {
    ...
    // bind discord
    try {
      const res = await post("/game/777/bindDiscord", {
    ...
    <div className={clsx("relative w-full text-center font-[DelaGothicOne] text-white text-[16px] leading-[120%] font-[400]", className)}>
      ...
      <div className={clsx("", isGetNew ? "pt-[150px]" : "pt-[30px]")}>
        ...
        <div className="mt-[33px] font-[SpaceGrotesk]">
          {isNft ? "You get the NFT" : `They noticed you. ${isNft ? `NFT ${nft?.category ? nft?.category + " " : ""}dispatched` : `WL ${nft?.category ? nft?.category + " " : ""}confirmed`}`}
        </div>
        {/* {
          isGetNew && (
