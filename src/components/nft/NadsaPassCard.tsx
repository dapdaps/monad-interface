                    ...
                    status !== 2 && <>
                        {
                            address ? <div className="flex items-center justify-between mt-[10px] text-[12px] h-[40px] px-[10px] bg-[#212041] rounded-[2px] font-Pixelmix">
                                ...
                                {
                                    isBindTwitterAccount && <div className="text-[#00FF00]">
                                        <RightArrow />
                                    </div>
                                }
                            </div> : <button onClick={() => {
                        ...
                        {
                            isBindTwitterAccount && <> {!isFollowedTwitter ? <button onClick={() => {
                                window.open('https://twitter.com/intent/follow?screen_name=0xNADSA', '_blank');
                                setTimeout(() => {
                                    setUserInfo({ isFollowedTwitter: true });
                                }, 3000);
                            }} className="flex relative items-center justify-center mt-[10px] text-[12px] w-full h-[40px] px-[10px] bg-[#7663F4] rounded-[2px] font-Pixelmix">
                                <div className="text-[#FFFFFF]">Follow 0xNADSA on X</div>
                                <div className="absolute right-[10px] top-[10px]" onClick={(e) => {
                                    e.stopPropagation();
                                }}>
                                    <EllipsisLoading key={isBindTwitterAccountLoading} />
                                </div>
...
const MainBtn = ({ onClick, children, disabled }: { onClick: any, children: any, disabled: boolean }) => {
    ...
    return (
        <button onClick={() => {
            ...
            onClick();
        }} disabled={disabled} className="flex w-full items-center justify-center text-[12px] h-[40px] mt-[10px] mb-[10px] bg-[#7663F4] text-[#fff] rounded-[2px] font-Pixelmix disabled:opacity-50">
            {children}
        </button>
    )
}
const ArrowIcon = () => {
    return (
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M8 2 A6 6 0 1 1 8.52 13.95" stroke="#00FF11" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M11 3 L13 1 L15 3" stroke="#00FF11" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M13 1 L13 5" stroke="#00FF11" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
    )
}
