...
export default function Route({
    ...
    return <div className={`py-[10px] text-white border  bg-[#151822] rounded-[4px] mt-[10px] cursor-pointer ${checked ? 'border-[#BFFF60]' : 'border-[#34304B]'}`} onClick={() => {
        onChange(true)
    }}>
        <div className="flex items-start gap-[10px] justify-between w-full whitespace-nowrap px-[10px]">
            <div>
                ...
                {
                    isBest && (
                        <div className="text-[12px] text-[#BFFF60] mt-[5px]">Best Price</div>
                    )
                }
            </div>
            ...
            <div className="flex-1">
                <div className="flex items-center justify-end gap-2 w-full">
                    <img className="w-[18px] h-[18px] rounded-[4px]" src={outputCurrency.icon || "/assets/tokens/default_icon.png"} />
                    <div className="text-[18px] max-w-[120px] font-[400] whitespace-nowrap overflow-hidden ">{receiveAmount}</div>
                </div>
        ...
        {
            (name === 'OneClick' || name === 'Monorail') && showDexInfo && <div className="pt-[10px] space-y-[10px] text-[10px] text-[#A1AECB] border-t border-[#34304B] mt-[10px] px-[10px]">
                {
                            ...
                            <div className="flex items-center gap-[4px] flex-1 justify-end">
                                {
                                    route.pools?.length > 0 && route.pools.map((pool: any, index: number) => (
                                        <div className="flex items-center gap-[4px] relative" key={index}>
                                            ...
                                            {
                                                Array.isArray(pool.dex) && <div className="absolute top-0 right-[7px] group">
                                                    <img className="w-[16px] h-[16px] rounded-[4px]" src='/images/mainnet/dex.jpg' />
                                                    <div className="text-[10px] text-white bg-[#00000080] rounded-[4px] px-[4px] py-[2px] absolute bottom-[100%] left-[50%] -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">{
                                                        pool.dex.map((dex: any) => {
                                                            return <div className="flex items-center justify-between gap-[4px] whitespace-nowrap">
                                                                <div>{dex.protocol}</div>
                                                                <div>{balanceFormated(dex.percentage, 2)}%</div>
                                                            </div>
                                                        })
                                                    }</div>
                                                </div>
